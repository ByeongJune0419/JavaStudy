...
public class CollectionList {
	public static void main(String[] args) {
		/*
		 * 컬렉션 프레임워크는 데이터를 보다 쉽게 다룰 수 있도록 재사용가능한 
		 * 컬렉션 클래스를 제공함
		 * List, Set, Map
		 * 1.List :순서대로 객체를 저장하는 구조, 중복된 원소를 허용
		 * 		   원소에 접근하기 위해 인덱스를 사용. (ArrayList, LinkedList)
		 * 2.Map  :key와 valuse 쌍으로 이루어진 객체를 저장하는 구조
		 * 		   key는 중복될 수 없으며 각 키는 하나의 값을 가리킴.(HashMap,TreeMap)
		 * 3.Set  :중복을 허용하지 않는 객체의 모음 원소의 순서는 보장되지 않음(HashSet,TreeSet)
		 */
		// 동적배열 (크기가 자유로움)
		...
		students.clear();       // 리스트 비율
		System.out.println(students);
		// shift + alt + a = 이클립스 편집 모드
		students.add("강지석");
		students.add("김도래");
		students.add("김병준");
		students.add("김서영");
		...
		stu.set(0, "지석");
		System.out.println(students);
		System.out.println(stu); //얇은 복사는 주소를 복사(공유하기)하기떄문에 둘다 같음
		// 2. 깊은 복사1
		...
		// <> <-- 이 부분은 제네틱(Genetic)이라하며 코드에서 타입의 안정성을 보장할 수 있으며
		// 불필요한 캐스팅을 줄이기 위해 만들어짐. 제네틱은 객체(참조타입)만 담을 수 있음.
		// 기본타입(int, float, boolean...)은 Wrapper class 사용 Intger, Bollean..
