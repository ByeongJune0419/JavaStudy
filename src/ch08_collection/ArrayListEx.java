...
public class ArrayListEx {
	public static void main(String[] args) {
		...
		System.out.println(searchSong("가수","데이식스",playList));
		System.out.println(searchSong("노래","무제",playList));
		//순위와 총 검색건수 출력
		Scanner scan = new Scanner(System.in);
		while(true) {
			System.out.println("탑100입니다. 가수 or 제목 어떤걸로 알려드릴까요?(종류:q)");
			System.out.print(">>>");
			String select = scan.nextLine();
			if(select.equalsIgnoreCase("q")) { //equalsIgnoreCase 대소문자 상관없이 같은지
				System.out.println("탑100 종료 =======");
				break;
			}
			...
			ArrayList<String> result = searchSong(select, keyword, playList);
			System.out.println(result);
			// 2. 없으면 결과가 없습니다. 있다면 총 몇건인지 출력 후 순서대로 출력
			// 없으면 (isEmpty() 로 체크 가능)
			if(result.isEmpty()) {
	...
	// input : 1.가수 or 노래, 2키워드, 3.리스트
	// output : 리스트 (검색된)
	// searchSong("가수","임영웅",playList); -> 임영웅 노래만 출력됨.
	public static ArrayList<String> searchSong(String option
			, String keyword, ArrayList<String> arr){
		// 전달받은 옵션을 기준으로 keyword 포함된 데이터를 찾아서 리턴
